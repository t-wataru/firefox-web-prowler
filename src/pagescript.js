...
window.addEventListener('mouseup', function (e) {  //mouseupでイベント発火
  var selectedStr;
...
async function page_register_request(textLines) {
  if (textLines == "") { return }
  if (page_register_request.textLines_before.length == textLines.length) { return }
...
function links_register_request() {
  ...
  for (a of document.getElementsByTagName("a")) {
    ...
    if (a_elem_by_url.size > 300) { break; }
  }
...
function recommend_request(text_request) {
  if (text_request == "") { return }
  if (recommend_request.textLines_before.length == text_request.length) { return }
...
function text_from_text_nodes_within_viewport() {
  const elems = Array.from(document.querySelectorAll("h1,h2,h3,h4,h5,h6,a,p,span,article"))
    .filter(element_is_in_viewport);
  const textLines = elems.map(e => Array.from(e.childNodes))
...
function text_in_html() {
  ...
  const og_contents = Array.from(document.querySelectorAll("meta[property*='og'")).map(og => og.content).join(" ");
  const innerText = document.querySelector("body").innerText;
  const textLines = (innerText + og_contents + title)
    .replace(/\s+|\n+/g, " ").replace(/\s+/g, " ")
    ;
  return textLines;
}