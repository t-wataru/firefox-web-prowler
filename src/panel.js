...
function init() {
    browser.runtime.onMessage.addListener(function (message) {
        if (message.type != 'display_related_page') {
            return;
        }
...
async function element_create(page, tokens) {
    ...
    const title_text = page.title.replace(/\n/g, ' ');
    const title_text_sliced = title_text.slice(0, TITLE_TEXT_LENGTH);
    ...
    text_bold_set_element_tokens(linkElement, title_text_sliced, tokens);
